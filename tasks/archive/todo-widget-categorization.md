# Todo: widget/ 分类目录化 + Switcher + showcase 分类导航
...
- [x] **Task 5** switcher.rs + 单元测试（clamp/尺寸/paint/event/sync/children)（依赖 4)— `cargo test widget::view::switcher` 6 绿 + clippy 零警告 ✅ 2026-07-21
- [x] **Task 6** tests/switcher.rs 集成测试（焦点链/切走清焦/event_at_path/点击隐藏区)（依赖 5)— `cargo test --test switcher` 5 绿 + 全量绿 ✅ 2026-07-21
...
- [x] **Task 8** 侧边栏 + Switcher 接线（依赖 7)— 人工验收通过（切换四类 / 交互回归 / 焦点语义 / Tab 循环）✅ 2026-07-21
