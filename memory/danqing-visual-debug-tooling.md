...
- **合成鼠标拖拽 (SetCursorPos+mouse_event 分步移动) 不到达应用** (2026-08-29 desk-window 验收实测): 注入序列 (窗口中心 DOWN → 10 步移动 → UP) 后 GetWindowRect 零位移, 应用日志无任何拖拽痕迹; 同日用户物理拖拽同一二进制完全正常 —— 即 drag_window 模态环与合成输入不兼容 (或注入本身被吞), 与上一条 SetWindowPos 失效同族。**拖拽类验收 = 用户物理过, 别在注入拖拽上诊断**; 需要脚本化点击时仍可试 click-post.ps1 (PostMessage 直投队列, 拖拽未验证)。
