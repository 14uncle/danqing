...
impl Theme for DarkTheme {
    ...
    fn selection(&self) -> Color {
        ...
        //
        // 浅色主题**不动**: 那里选区带合成后是**中间的青** (线性合成 **(205,219,217)**,
        // 比暗色那支浅得多), 深浅两档前景都还压得住。
        //
        // **但别拿一支以偏概全** —— 2026-09-13 复核时发现这里原先只量了「ERROR 红
        // 4.04」就下了「中亮度前景本来就够」的结论, 而实测浅色的 WARN / DEBUG 压在
        // 它上面只有 **2.45 / 2.34**。原因不在选区带: 是产品侧那套语义色**只对白底
        // 成立**。那一侧修在**色板**上 (danqing-log 的 `LevelPalette` 与
        // `light_semantic_colors_clear_wcag_aa_on_light_surfaces`), 修完压选中行
        // 3.82–4.04 —— 结论「浅色不动」到这时才真正有实测撑着。
        //
