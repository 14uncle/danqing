...
- **里程碑状态**: M1~M3(渲染/焦点/滚动)+ 阶段 1(设计系统与品牌视觉)+ 阶段 2(番茄钟 POC,场景沉浸美学)+ 阶段 2 补完(5 个真缺全部修复)+ 番茄钟打磨三件套(场景环境音 / 长休息+轮次 / 今日完成计数,2026-07-28 人工终审全过)+ 托盘与提示(2026-07-28 人工终审关闭)+ 雨场景动效试点(2026-07-28 人工终审关闭)+ 篝火场景动效(2026-07-29 人工终审关闭)+ 海场景动效(2026-07-29 人工终审关闭)+ 雨场景改造(2026-07-29 人工终审关闭)+ 山/森林场景动效(2026-07-30 代码+门槛全绿,人工终审待用户在 showcase 上跑)均已关闭并归档到 `tasks/archive/`。番茄钟现已可日常使用:最小化到任务栏常驻,全局热键呼出/暂停,关闭再开状态完整恢复,阶段流转有视觉+听觉反馈,5 场景 CC0 环境音随画面交叉淡化、暂停 300ms 沉降(rodio 懒初始化 + 静默降级,响度统一 -28 LUFS),雨场景静态图已去丝、程序化雨幕(~290 丝三层)独挑、暂停雨钟冻结雨丝定格可见,篝火场景光晕呼吸 + 余烬上浮、海场景波带 UV 位移起伏 + 波光碎点明灭、山场景暮色径向光呼吸 + 山脊 silhouette 慢呼吸、森林场景顶光呼吸 + 两道横雾水平 UV 漂移反向造穿林风,五效果共用 500ms MotionEnvelope 暂停回静态(shader 程序化,零新资产,uniform 36B 9×f32 全标量每效果一标量 + 雨钟;山/森林雾漂移在 0.008~0.012 区间为终审调参点)。
- **当前分支**: `dev`(主分支 `master`);便携包诊断日志已落地,规格见 `docs/specs/portable-diagnostics-logging.md`。
- **下一步**: 山/森林场景动效已按"静态图去烘焙 + 运行时程序化"范式落地,等待用户人工终审(运行+暂停帧差截图归档);代码+门槛全绿,benchmark startup 762ms / WS 274MB PASS。下一里程碑候选:第二 POC(剪贴板历史管理器,效率工具族,美学剂量低于专注陪伴族),或用户另行指定。**未获用户指示时不要启动新 POC**。
