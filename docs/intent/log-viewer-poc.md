...
| VS Code 扩展 daucloud.json-viewer | 免费 | 活跃 | GB 级虚拟化表格 + JSON Pointer 过滤——**最锋利的免费对手**；1GB 日志冷启动 ~7 秒（VS Code 宿主+扩展激活开销），JSONL demo 必须显著优于它 |
...
> **2026-09-12 重裁**：本段原为「v1 单二进制全功能 / $45 正面贴 LogViewPlus」，已作废。
> 裁因：四张对比图逐项核实后确认功能差距为**真缺四项**（多文件/标签、等级直方图、导出、会话模板）
> —— $45 恰是 LogViewPlus 的价格，同价会让用户带着「它有的我都有」的预期进来，而预期必然落空。
> 免费层/付费层与渠道的完整清单见 `danqing-log/docs/ROADMAP-v1x.md`（本段只留裁决与理由）。

**分层原则：免费层 = 看懂；付费层 = 批量 / 留存 / 交付。**

| 层 | 内容 |
|---|---|
| 免费层 | 单文件、mmap 秒开、虚拟滚动、正则搜索高亮、tail+实时过滤、JSONL 列化+嵌套展开、编码检测、书签、等级直方图 |
| 付费层 | 多文件时间戳合并（+跨文件 `req_id` 跳转）、字段分析（数值列 p50/p99/分布）、导出、工作台会话持久化 |

- 免费层是营销预算 —— 开源即获客引擎，对标 klogg 抢「停更 4 年」的接盘流量。
- 付费层两条腿**均需 v1.x 新建**；字段分析复用已有 memmem 字段提取，成本低于多文件合并一个量级。
- 凡属 **v1 欠账**者（列重排、书签持久化）一律进免费层 —— 当付费钩子会被读成「扣走我本来就该有的东西」。

**渠道分层 —— 同一份代码，两套收费**

- **GitHub 主**：免费层永久免费开源；付费层 **$29** 买断（2026-09-12 定档 —— 免费层已给出
  LogViewPlus 七成功能，$29 让价格与事实一致；$39 会重新引入「同价比较」的压力，而功能少一档答不上来）。
- **MS Store 辅**：**v1.0 起以纯免费层上架**（只做打包 + 上架，无 trial、无内购）；付费层走 Store
  内置 trial → 买断，**留 v1.x**。**试用过期 = 降级到免费层，不是变砖。**
  （LogoRRR Pro 在架证明类目存在且稀薄；商店用户习惯 trial→buy，且不在意开源）
  > 2026-09-12 用户裁决「MS Store 纳入 v1.0，形态为纯免费层上架」。接 trial 等于把付费层基建
  > （授权轨 / 过期降级）前移进 v1.0，正是下方三条裁因里「试用期撞生产事故」被否决的那件事。
  > **连带推论**：v1.0 两个渠道都是免费层 → 无购买路径 → **前提③（首单外检）只能在 v1.x
  > 付费层上线后判定**，原「MS Store 试转付费的首单即可判定」不再适用于 v1.0。
- **两档结构**：个人 **$29** / 企业 **$59/seat**（2026-09-12 定档）。企业档按 LogViewPlus
  个人↔企业的 2.1 倍差折算 —— 原 $95 在个人档降到 $29 后成 3 倍差，过陡。买断，不设订阅。
- **不设订阅**：Dadroit 按文件大小收订阅正是其口碑裂缝，反向操作即差异化。

**为什么不用「v1.0 全功能 + 试用期」单轨**（2026-09-12 否决）：
① 多文件合并是引擎+UI 双改，全吞进 v1.0 = 发布再推迟，而前提③已等久；
② 丢掉 GitHub 开源叙事 —— 整条冷启动话术建立在开源免费上，试用制产品讲不了；
③ **品类特有的时机风险**：日志工具的使用是间歇性的，用户最需要它的时刻是「线上出事了」。
试用期恰在此时过期 = 把「最需要的时候抛弃我」永久绑到产品上。日志查看器和灭火器一样，
可靠性情绪的权重极高，不适合试用制。（MS Store 轨保留 trial 无此风险 —— 因为过期只降级、不变砖）
