...
- **B「Hazel for Windows 文件自动整理」**：候选头名，等 log 发布后用户重启 —— **2026-09-15 用户指令从候选队列去掉，此触发条件作废**。
- 枪毙归档：泛截图工具（红海）、AI 听写（空窗已关）、快速记录（入口被掐）、抓包（团队赛道）、数字健康阻断器（MSIX 风险）。

## 裁决（2026-09-15 用户指令）

**埋掉**：不立项、不推进；仓库 `danqing-disk/` 与代码/spec/plan 产物原样保留作档案（同 Xirang 处置）。代码虽已完成（22 task 全勾 / 91 测试绿），全流程验收（15 项 Checkpoint 含「发布就绪」）从未执行，从未发布。

**死因（用户 2026-09-15 亲述）**：本书在**旧指南**下选定（2026-09-11），未经过充分调研；事后判定为**重投入产品，且投入与回报不等价**。埋点早于人工验收——产品从未被造物主实际使用过。

**流程侧注（agent 补充，非用户原话）**：09-11 选定时，09-15 才补上的三道闸（裁决段 / 红队 / 一页粗算）与缝隙判据修正均尚不存在。其中「**一页粗算**」（渠道可见流量 × 转化 × 单价 vs 预计工时）正是能提前挡住「重投入、回报不等价」的那一闸——本案例是该闸的第一份实证：它不是仪式，是可以救命的。

**体检结果（2026-09-15，双 agent 查证，全部为实测数据）**：

- **在位者状态：当年记录全面失真。** WinDirStat 近 12 个月 **10 次发布**（最新 beta 2026-09-14），2.5.0（2026-01-18）已加 `direct NTFS MFT scanning` + 深色模式，UI 由 MFC 重写为 Win32/GDI，**2026-05-23 上架 MS Store**（4.9★/30 评分）。记录里那三条「致命短板」，在选型日之前已被修掉两条半（速度、暗色、危险操作确认；重复文件检测亦已有）。另：WizTree 深色模式自 **2021-02 的 3.36** 即存在；「#106 七年未实现」实为 2024-10-22 开、**2026-01-18 已 completed 关闭**。
- **定位已被在位者自己完成**：原计划的四条差异点（开源 / MFT 极速 / 现代 UI / 免费）**已经是 WinDirStat v2 自己的配置**；冷启动口号「开源版 WizTree + 现代 UI」被在位者亲手实现，且同步占据了 MS Store 渠道。另见新入局者 MangoDisk（Rust + Tauri，2026-08-01 建仓，2,678★）。
- **免费封死**：WizTree 个人版 EULA 原文 PERSONAL USE ONLY，12 个月 5 次更新；付费档 $25 起按企业员工数分档（$25–$1800），2025-26 无授权变更。
- **需求侧：痛点真实，但四条「铁证」成色须下调。** 开发者缓存膨胀扎实（硬行为证据：npkill 9,443★ / npm 上月 5.26 万次下载；node-prune 3.46 万次/月）；但 WSL#4699「7 年未解决」**不成立**——微软 2023-09-18（WSL 2.0.0）即内置 `wsl --manage <distro> --set-sparse` 并持续维护至 2026-08，且该 issue 的 1418 是**总反应数**（纯 👍 1188）；SSD 性能悬崖是**媒体/厂商技术说明，不是用户声音**（真实用户提问集中在 2017-2018）。
- **渠道实测挂科**：Show HN 同类发布 12 个月内 14+ 次、**中位数约 2 分**；MS Store 品类头部全为免费应用，`disk space analyzer` 前 20 有 8 个零评分；付费位唯一实存为 TreeSize（27 评分，其免费孪生兄弟 146 评分）。
- **付费真相**：品类里真实存在的钱只有两处——① B2B 存储管理 ② **商业合规授权**（WizTree Supporter Code）。**面向个人的价值费：零正面实证。**

**事后算账（补做 09-11 欠的那一页）**：渠道可见流量 ≈ 0，乘任何转化与单价仍是 0；而维护面（MFT 提权 / 云文件占位符 API / 百万节点 treemap / SQLite 快照 / Blake3 去重 / 授权码 / CLI）是长期负债。「重投入、回报不等价」——**得证**。
