...
> 共同摩擦: clipboard 用 `windows` 0.62, danqing 用 `windows-sys` 0.59 —— E1/E2/E4 需移植 + 非 Windows stub。
...
| E3 | `to_crlf` 文本归一 | 裸 LF → CRLF (Windows 剪贴板惯例, 旧版记事本换行丢失) | clipboard `inject.rs:15-20` | **S** | 4 测试纯逻辑, 零依赖 |
...
- **E3 是纯函数**: 4 测试、零依赖, 「任何写剪贴板文本的产品必遇」。
...
1. **单元测试直接搬** (行为保持):
   ...
   - E3: 4 测试 (裸 LF / 已含 CRLF / 孤立 \r / 空串) 直接搬, 零改动。
...
3. **产品迁移验证** (clipboard):
   ...
   - E3: `inject.rs` 删手写 `to_crlf`, 改 `danqing::text::to_crlf`。
   - 既有测试零改动全绿 (行为保持)。

## Boundaries

- **Always**: 失败静默降级不 panic (前台/剪贴板 API 在无 GUI/权限不足时返回 None/降级); 非 Windows stub 保编译 (产品边界 Windows-only); 隐私判定与落库是产品业务, 框架只提供读取原语。
