...
- **波带涌动**: 已有波带区域的亮度缓慢起伏,亮流沿水平方向行进(乘性调制,只起伏已有波带,不移动剪影边缘、不改色相);
- **波光碎点**: 波带区域内散布的细碎光点缓慢明灭(同静态图低对比语言,形小、量少、速度慢)。

**与篝火的关键差异 —— 亮场景**: 篝火是暗场景,余烬用 additive 暖色点可读;海的波带是中亮青、天空近白,additive 光点会被近白底吃掉。海效两组件均以**乘性**调制为主(提亮已有像素),这是亮场景下的形态适配,shader 叠加路径相应不同(记录,终审裁定)。
...
- **涌动**: 2 层水平行进的正弦亮度波叠加(时间频率取 1/8 Hz 整数倍,空间频率错开),mask = 波带区域纵向软入(uv.y ≈ 0.60 起渐入,0.75 全量,对齐静态图波带上缘);`color.rgb *= 1 + swell × mask × sea_intensity`(乘性,幅度 ±3~5% 量级起步)。
- **碎点**: 分列 hash(同雨/余烬的分列范式),每列相位随机;位置在波带区域内基本不动(水波光点在原地明灭,不漂移),亮度按低频正弦缓慢明灭(频率取 1/8 Hz 整数倍,各列相位错开);2D 软圆点(宽高比修正,同余烬),乘性提亮;点直径 3~5px @960px 窗宽起步,同屏 ~15~25 颗,亮流带内软裁。
...
1. **碎点叠加方式**: 亮场景下 additive 易被近白底吃掉,拟用乘性提亮(与涌动同路径)。备选: additive 但增益放大。——待定,终审目测裁定。
2. **波带区域锚点**: 涌动 mask 纵向软入 uv.y 0.60→0.75、碎点散布带 uv.y 0.72~0.98(对齐静态图三叠波带)。——待定,实现时按抓帧校准。
3. **碎点是否漂移**: 拟原地明灭(水波光点不位移,实现更简单、公共周期更稳)。备选: 极慢水平漂移。——待定,终审目测裁定。
