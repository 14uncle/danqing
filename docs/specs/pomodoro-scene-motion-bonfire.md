...
(全部以假设落定,待用户确认;确认即转决议)

1. **效果概念**: 光晕呼吸 + 火星余烬上浮,不画火焰形状(静态图本身无火体,火在画框外)。——假设,待确认。
2. **锚点**: 光晕中心 uv ≈ (0.5, 0.95),余烬发射带 uv.x 0.35~0.75、自底部升起至 y≈0.35 淡出(对齐静态图火星散布带)。初值,调参轮可动。
3. **uniform 形状**: 每效果一标量(32B),不引入效果选择子枚举。——设计决议,记录理由(交叉淡化期间双效果并存)。
4. **时序**: 火效与雨共享 `time` + 8s 公共周期;火效频率全部落在 1/8 Hz 整数倍网格上。——设计决议。
5. **沉降**: 雨火共用同一 `MotionEnvelope` 实例(同涨同落),不为火单设包络。——假设,待确认。
