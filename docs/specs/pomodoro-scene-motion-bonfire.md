...
2. **锚点**: 光晕中心 uv ≈ (0.5, 0.95), 余烬发射带 uv.x 0.35~0.75、自底部升起至 y≈0.35 淡出(对齐静态图火星散布带)。——决议: 采纳, 终审未再调整。
   - **2026-07-29 追记 (火床重定位)**: 用户指出余烬从屏幕底边升起与静态火床位置不一致——真实物理中火星应从火堆(余烬/光晕)上方冒出。火床从 y≈0.95 重定位到 y≈0.86,静态图同步移动(glow center 0.74→0.86, embers 0.35~0.70→0.55~0.88),shader 余烬起点从 `1.0 - life*SPAN` 改为 `FIRE_CENTER.y - life*SPAN`,呼吸半径 0.55→0.48,余烬行程 0.65→0.52。改后静态火床与 shader 火床在同一位置,火星从火床升起物理一致。
