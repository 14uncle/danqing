...
def _spectrum(freqs: np.ndarray, kind: str) -> np.ndarray:
    ...
    if kind == "snow":  # 雪原: quiet airy hiss, high-freq dusting
        air = (freqs / 2400.0) ** 1.6 / (1.0 + (freqs / 2400.0) ** 2.0)
        low = 1.0 / (1.0 + (freqs / 500.0) ** 2.0) * 0.25
        return np.clip(air * 0.85 + low, 0.0, 1.0)
